    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="card card-pad">
                ...
                <div class="mb-4">
                    <div class="text-2xl font-semibold text-gray-50 capitalize">{{ ucfirst($user->plan) }}</div>
                    @if($user->plan === 'free')
                ...
                <div class="mt-4">
                    <form action="{{ route('billing.cancel-subscription') }}" method="POST"
                          ...
                          data-confirm-button="Cancel Subscription">
                        @csrf
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="alert alert-warning items-start">
                <div class="min-w-0">
                    <h2 class="font-semibold mb-1">Pending Upgrades</h2>
                    <p class="text-xs mb-3 opacity-90">These are upgrade requests awaiting payment confirmation from 2Checkout. If you completed payment but your plan hasn't updated within 10 minutes, please contact support with your invoice ID.</p>
            ...
            <div class="card card-pad">
                <h2 class="eyebrow mb-4">Transaction History</h2>

                @if($transactions->isEmpty())
                    <div class="empty-state">
                        <svg class="w-10 h-10 text-gray-500 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                        ...
                        <table class="table-base">
                            <thead class="table-head">
                                <tr>
                                    ...
                                    <th class="table-head-cell">Invoice ID</th>
                            ...
                            <tbody>
                                ...
                                <tr class="table-row-base">
                                    <td class="table-cell text-gray-300 text-numeric">{{ $tx->created_at->format('M j, Y') }}</td>
                                    <td class="table-cell text-gray-300 capitalize">{{ $tx->plan }}</td>
                                    <td class="table-cell text-gray-300 text-numeric">{{ $tx->formattedAmount() }}</td>
                                    <td class="table-cell text-gray-500 font-mono text-xs break-all">{{ $tx->invoice_id }}</td>
                                    <td class="table-cell">
                                        ...
                                        @else
                                            <span class="badge badge-neutral">{{ ucfirst($tx->status) }}</span>
                                        @endif
                                    </td>
                                    <td class="table-cell">
                                        @php
            ...
            <div class="pt-2">
                ...
                <a href="{{ route('refund') }}" class="action-link">Read our refund policy →</a>
            </div>
