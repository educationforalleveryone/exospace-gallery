<x-guest-layout>
    <div class="text-center mb-6">
        <div class="w-14 h-14 bg-purple-600/20 border border-purple-500/30 rounded-xl flex items-center justify-center mx-auto mb-3">
    ...
    @if (session('status') == 'verification-link-sent')
        <div class="mb-4 p-3 bg-green-900/40 border border-green-700/50 rounded-lg text-sm text-green-300 text-center">
    ...
    <div class="mb-6 bg-gray-900/50 border border-gray-700 rounded-xl p-4 space-y-3">
        ...
        <div class="flex items-center gap-3 text-sm text-gray-300">
            <div class="w-8 h-8 rounded-lg bg-purple-600/15 flex items-center justify-center flex-shrink-0">
                <svg class="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            </div>
            <span>Build immersive 3D art exhibitions</span>
        </div>
        <div class="flex items-center gap-3 text-sm text-gray-300">
            <div class="w-8 h-8 rounded-lg bg-indigo-600/15 flex items-center justify-center flex-shrink-0">
    ...
    <div class="flex items-center justify-between gap-3">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="text-sm text-gray-500 hover:text-gray-300 transition px-3 py-2 rounded-md">
                Log out
            </button>
        </form>
    </div>
