...
<section class="max-w-5xl mx-auto px-4 py-16">
    <div class="bg-gradient-to-br from-brand-900/40 to-gray-900 border border-brand-800/40 rounded-2xl p-10 text-center">
        @if(!empty($data['title']))
        ...
        @if(!empty($data['button_text']) && !empty($data['button_url']))
            <a href="{{ $data['button_url'] }}" class="inline-flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-brand-600 to-brand-600 hover:from-brand-500 hover:to-brand-500 text-white font-semibold transition">
                {{ $data['button_text'] }}
