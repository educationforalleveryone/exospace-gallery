...
<section class="bg-gradient-to-br from-gray-900 via-purple-950/30 to-gray-900 border-b border-gray-800">
    <div class="max-w-5xl mx-auto px-4 py-20 text-center">
        @if(!empty($data['eyebrow']))
            <p class="text-purple-400 text-sm font-semibold tracking-widest uppercase mb-4">{{ $data['eyebrow'] }}</p>
        @endif
        ...
        @if(!empty($data['cta_text']) && !empty($data['cta_url']))
            <a href="{{ $data['cta_url'] }}" class="inline-flex items-center gap-2 mt-8 px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold transition">
                {{ $data['cta_text'] }}
