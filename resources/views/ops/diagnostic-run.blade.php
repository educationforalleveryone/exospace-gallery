    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-1.5">
                <span class="text-[10px] font-bold px-2 py-1 rounded border {{ $statusStyles['chip'] }}">{{ $statusStyles['text'] }}</span>
                <span class="text-[10px] px-2 py-1 rounded bg-slate-800/80 text-slate-400 font-mono">{{ $run->diagnostic_id }}</span>
                <span class="text-[10px] px-2 py-1 rounded bg-slate-800/80 text-slate-400">run #{{ $run->id }}</span>
            </div>
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-4">What was checked</h2>
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-3">What this means</h2>
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-3">Suggested next steps</h2>
    ...
    <div class="space-y-6">
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-3">Run details</h2>
            <dl class="space-y-2 text-xs">
                <div class="flex justify-between gap-2"><dt class="text-slate-500">Diagnostic</dt><dd class="text-slate-200 font-mono text-[10px]">{{ $run->diagnostic_id }}</dd></div>
        ...
        @if($definition)
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-2">About this diagnostic</h2>
            <p class="text-[11px] text-slate-500 leading-relaxed">{{ $definition['description'] }}</p>
        </section>
        @endif

        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-2">Safety</h2>
