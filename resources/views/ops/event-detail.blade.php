    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-1.5">
                <span class="text-xs font-semibold px-2 py-1 rounded border {{ $sev['chip'] }}">{{ $sev['text'] }}</span>
                <span class="text-xs font-mono px-2 py-1 rounded bg-slate-800 text-slate-400">{{ $event->category }}</span>
                <span class="text-xs px-2 py-1 rounded bg-slate-800 text-slate-400 uppercase">{{ $event->source }}</span>
                @if($event->status !== 'open')<span class="text-xs px-2 py-1 rounded bg-slate-900 text-slate-500 border border-slate-800">{{ $event->status }}</span>@endif
            </div>
            <h1 class="page-title text-slate-50">{{ $event->title }}</h1>
        </div>
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-3">What happened</h2>
            @if($event->message)
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-3">Why it matters</h2>
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-3">Likely causes</h2>
            <ul class="space-y-2">
            ...
            <p class="text-xs text-slate-600 mt-3">Causes are ranked possibilities, not certainties — verify with diagnostics before acting.</p>
        </section>

        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-3">Recommended next steps</h2>
            @php
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-3">Related events on the same application</h2>
            <p class="text-xs text-slate-500 mb-3">Events from this application near this event's window. When they share a cause, the correlation engine groups them into one incident.</p>
            <div class="divide-y divide-slate-800/70 rounded-lg border border-slate-800 overflow-hidden">
                @foreach($related as $rel)
                    <a href="{{ route('ops.events.show', $rel) }}" class="block p-3 hover:bg-slate-900/80 text-sm">
                        <span class="text-xs font-semibold px-1.5 py-0.5 rounded mr-2 {{ $rel->severity === 'critical' ? 'bg-red-950/60 text-red-300' : ($rel->severity === 'error' ? 'bg-orange-950/50 text-orange-300' : 'bg-amber-950/50 text-amber-300') }}">{{ strtoupper($rel->severity) }}</span>
                        <span class="text-slate-200">{{ $rel->title }}</span>
                        <span class="text-xs text-slate-500 ml-2">{{ $rel->last_seen_at?->diffForHumans() }}</span>
                    </a>
                @endforeach
            </div>
        </section>
        ...
        <details class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <summary class="text-xs font-semibold uppercase tracking-wider text-slate-500 cursor-pointer select-none">Technical context (already redacted)</summary>
            <div class="mt-4 space-y-4">
                <div>
                    <h3 class="eyebrow text-slate-500 mb-1.5">Facts</h3>
                    <dl class="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
                        ...
                        <div class="flex justify-between border-b border-slate-800/60 py-1"><dt class="text-slate-500">Fingerprint</dt><dd class="text-slate-200 font-mono text-xs">{{ \Illuminate\Support\Str::limit($event->fingerprint, 16) }}</dd></div>
                    </dl>
                </div>

                @if(data_get($event->context, 'http'))
                    <div>
                        <h3 class="eyebrow text-slate-500 mb-1.5">HTTP context</h3>
                ...
                @if(data_get($event->context, 'stack'))
                    <div>
                        <h3 class="eyebrow text-slate-500 mb-1.5">Stack excerpt (first frames)</h3>
                ...
                @if($event->context && data_get($event->context, 'http') === null && data_get($event->context, 'stack') === null)
                    <div>
                        <h3 class="eyebrow text-slate-500 mb-1.5">Context</h3>
                ...
                <p class="text-xs text-slate-600">Full stack traces remain in Sentry and the daily log files — this store keeps redacted excerpts only.</p>
            </div>
        </details>
    </div>
    ...
    <div class="space-y-6">
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            ...
            <div class="text-sm text-slate-200 font-medium flex items-center gap-2">
                ...
                @if($event->application?->is_self)<span class="text-xs px-1.5 py-0.5 rounded bg-cyan-900/50 text-cyan-300 border border-cyan-700/40 font-semibold">SELF</span>@endif
            </div>
            @if($event->application)
                <dl class="mt-3 space-y-1.5 text-xs">
                    ...
                    @if($event->application->url)
                        <div class="flex justify-between"><dt class="text-slate-500">URL</dt><dd class="text-slate-300 truncate max-w-[140px] font-mono text-xs">{{ $event->application->url }}</dd></div>
                    @endif
                    <div class="flex justify-between"><dt class="text-slate-500">Coolify status</dt><dd class="text-slate-300 font-mono text-xs">{{ $event->application->status }}</dd></div>
                </dl>
                <a href="{{ route('ops.applications') }}" class="text-xs text-emerald-400 hover:text-emerald-300 mt-3 inline-block">View application →</a>
            @endif
        </section>

        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">How often</h2>
            <div class="text-2xl font-semibold text-slate-100 text-numeric">{{ $event->occurrence_count }}</div>
        ...
        <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">What changed</h2>
            @php
