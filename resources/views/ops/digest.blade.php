...
<div class="mt-4 text-[11px] text-slate-600 space-y-1">
    ...
    <p>Manual sends are super-admin only, throttled and audited (<span class="font-mono">ops.digest.sent</span>). They deliberately bypass the daily dedup: a test send that silently disappeared would look exactly like a broken webhook.</p>
    <p>The digest records no ops_events rows — it reports on events, it must not become one.</p>
</div>
@endsection
