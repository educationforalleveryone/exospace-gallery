...
<div class="max-w-3xl mx-auto">
    ...
    <div class="rounded-xl border border-amber-700/60 bg-amber-950/30 p-5 mb-6">
        <div class="flex items-center gap-2 mb-2">
            <span class="text-[10px] font-bold px-2 py-1 rounded bg-amber-900/70 text-amber-200 border border-amber-700/60">CONFIRMATION REQUIRED</span>
            <span class="text-[10px] font-bold px-2 py-1 rounded bg-amber-950/60 text-amber-300 border border-amber-800/50">RISK: ELEVATED</span>
        </div>
    ...
    <section class="rounded-lg border border-slate-800 bg-slate-900/40 p-5 mb-6">
        <h2 class="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-3">Target</h2>
        @if($application)
            <div class="text-sm text-slate-100 font-medium flex items-center gap-2">
                {{ $application->name }}
                @if($application->is_self)<span class="text-[9px] px-1.5 py-0.5 rounded bg-cyan-900/50 text-cyan-300 border border-cyan-700/40 font-semibold">SELF — CONTROL PLANE HOST</span>@endif
            </div>
            <dl class="mt-3 grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
                ...
                <div class="flex justify-between border-b border-slate-800/60 py-1"><dt class="text-slate-500">Coolify status</dt><dd class="text-slate-300 font-mono text-[10px]">{{ $application->status }}</dd></div>
                ...
                <div class="flex justify-between border-b border-slate-800/60 py-1"><dt class="text-slate-500">Resource UUID</dt><dd class="text-slate-300 font-mono text-[10px]">{{ \Illuminate\Support\Str::limit($application->provider_uuid ?? ($application->meta['coolify_uuid'] ?? '—'), 18) }}</dd></div>
            </dl>
            @if($application->is_self)
                <p class="text-[11px] text-cyan-300/80 mt-3">⚠ This is the application the control plane itself runs in — restarting it briefly takes the dashboard down with it. The restart request is accepted before that happens, and the audit + Slack records persist either way.</p>
            @endif
        @elseif($webhook)
            <div class="text-sm text-slate-100 font-medium">Webhook #{{ $webhook->id }} — {{ $webhook->message_type ?? 'unknown type' }}</div>
            <dl class="mt-3 grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
                <div class="flex justify-between border-b border-slate-800/60 py-1"><dt class="text-slate-500">Invoice</dt><dd class="text-slate-300 font-mono text-[10px]">{{ $webhook->invoice_id ?? '—' }}</dd></div>
                <div class="flex justify-between border-b border-slate-800/60 py-1"><dt class="text-slate-500">Current status</dt><dd class="text-slate-300 font-mono text-[10px]">{{ $webhook->status }}</dd></div>
    ...
    <div class="grid md:grid-cols-2 gap-4 mb-6">
        <section class="rounded-lg border border-red-900/50 bg-red-950/20 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-red-400 mb-3">This WILL</h2>
            <ul class="space-y-2">
        ...
        <section class="rounded-lg border border-emerald-900/50 bg-emerald-950/20 p-5">
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-3">This will NOT</h2>
            <ul class="space-y-2">
    ...
    <section class="rounded-lg border border-amber-900/50 bg-amber-950/20 p-5 mb-6">
        <h2 class="text-[11px] font-bold uppercase tracking-wider text-amber-400 mb-2">Bottom line</h2>
    ...
    <form method="POST" action="{{ route('ops.actions.execute', $actionId) }}" class="rounded-lg border border-slate-800 bg-slate-900/60 p-6 space-y-5">
        ...
        <div>
            ...
            <p class="text-[11px] text-slate-500 mt-1.5">Required fresh for every elevated action — session confirmation is not carried over.</p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3 pt-2 border-t border-slate-800/70">
            ...
            <span class="text-[11px] text-slate-500">This will be audited and announced in Slack.</span>
        </div>
    </form>
</div>
@endsection
