...
<section class="rounded-xl border border-slate-800 bg-slate-900/40 p-5 mb-6">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div class="text-right">
            ...
            <div class="text-[10px] uppercase tracking-wider text-slate-500">{{ strtoupper($healthScore['band']) }}</div>
        </div>
    </div>
        ...
        @foreach($healthScore['components'] as $key => $component)
            <div class="rounded-lg border border-slate-800 bg-slate-950/50 p-3">
                <div class="flex items-baseline justify-between gap-2">
                    ...
                    <span class="text-[10px] text-slate-500">{{ $component['weight'] }}%</span>
                </div>
                ...
                <div class="mt-1.5 text-[11px] {{ $component['score'] >= 90 ? 'text-emerald-300' : ($component['score'] >= 70 ? 'text-amber-300' : 'text-red-300') }}">
                    {{ $component['score'] }}/100
                </div>
                <ul class="mt-2 space-y-1">
                    @foreach($component['reasons'] as $reason)
                        <li class="text-[10px] text-slate-400 leading-snug flex gap-1"><span class="text-slate-700">·</span>{{ $reason }}</li>
                    @endforeach
                </ul>
            </div>
        @endforeach
    ...
    @if(! empty($healthScore['applied_caps']))
        <div class="mt-4 rounded-lg border border-amber-800/50 bg-amber-950/30 px-4 py-3">
            <div class="text-[10px] font-bold uppercase tracking-wider text-amber-300 mb-1.5">Verdict caps applied</div>
            <ul class="space-y-1">
                @foreach($healthScore['applied_caps'] as $cap)
                    <li class="text-[11px] text-amber-200/90 flex gap-2"><span class="text-amber-600">▸</span>{{ $cap }}</li>
                @endforeach
            </ul>
