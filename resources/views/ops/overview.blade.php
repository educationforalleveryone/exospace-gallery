    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex gap-3 text-center">
            <div class="rounded-lg bg-slate-950/60 border border-slate-800 px-4 py-2">
                <div class="text-xl font-semibold text-numeric {{ ($openCounts['critical'] ?? 0) > 0 ? 'text-red-400' : 'text-slate-400' }}">{{ $openCounts['critical'] ?? 0 }}</div>
                <div class="text-xs uppercase tracking-wider text-slate-500">Critical</div>
            </div>
            <div class="rounded-lg bg-slate-950/60 border border-slate-800 px-4 py-2">
                <div class="text-xl font-semibold text-numeric {{ ($openCounts['error'] ?? 0) > 0 ? 'text-orange-400' : 'text-slate-400' }}">{{ $openCounts['error'] ?? 0 }}</div>
                <div class="text-xs uppercase tracking-wider text-slate-500">Errors</div>
            </div>
            <div class="rounded-lg bg-slate-950/60 border border-slate-800 px-4 py-2">
                <div class="text-xl font-semibold text-numeric {{ ($openCounts['warning'] ?? 0) > 0 ? 'text-amber-400' : 'text-slate-400' }}">{{ $openCounts['warning'] ?? 0 }}</div>
...
<section class="mb-6">
    <div class="flex items-center justify-between mb-3">
        <h2 class="text-xs font-semibold uppercase tracking-wider text-red-400">Active Incidents — Correlated Stories</h2>
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <section class="lg:col-span-2 space-y-6">
        <div>
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-400">Application Health</h2>
                            ...
                            <div class="flex items-center justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="text-xs font-semibold px-1.5 py-0.5 rounded border {{ $sev[2] }} {{ $sev[0] }} {{ $sev[1] }}">{{ $sev[3] }}</span>
    ...
    <section class="space-y-6">
        ...
        <div class="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
            <h2 class="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">Deployments (24h)</h2>
