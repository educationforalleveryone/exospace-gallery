...
//
// RANGE-REQUEST GUARD (v3):
//   • "Failed to execute 'put' on 'Cache': Partial response (status code 206)
//     is unsupported" — audio/video elements issue Range requests; the
//     network responses come back 206 Partial Content, and `response.ok`
//     is TRUE for 206 (ok = status 200-299), so Strategy 4 happily tried
//     to cache.put() them — Cache API rejects partial responses (the
//     spec only allows 200 + opaque), rejecting the promise and spamming
//     "Uncaught (in promise)" on every gallery with audio. Two guards:
//       1. Range requests now bypass the SW entirely (browser handles
//          them natively — correct behaviour for seekable media).
//       2. Every cache.put is gated on `response.status === 200` —
//          `response.ok` alone is not enough.
//
// STALE-WHILE-REVALIDATE FOR ENGINE ASSETS (v2):
//   • The gallery view registers this SW too — that's where the heavy
//     engine assets live (three.js chunk, DRACO/Basis wasm, HDRIs,
//     artwork textures).
...
// Do NOT pre-cache a literal '/build/assets/app.css' path.
//
// The built CSS is content-hashed (e.g. assets/app-_q6yfwqq.css)
// per the Vite manifest; the literal path 404s, and pre-caching it
// would serve a broken stylesheet from cache on the next visit.
//
...
self.addEventListener('fetch', (event) => {
    ...
    // Skip Range requests (audio/video seeking) — responses come back
    // 206 Partial Content, which the Cache API refuses to store. Passing
    // them through unhandled lets the browser's own media cache do its
    // job. (Post-deploy hotfix v3 — see the version note above.)
        ...
        event.respondWith(
            caches.match(request).then((cached) => {
                return cached || fetch(request).then((response) => {
                    // status === 200 only — response.ok also accepts 206,
                    // which cache.put rejects (hotfix v3).
    ...
    // /storage/ (artwork uploads), /assets/ (engine textures,
    ...
    if (url.pathname.startsWith('/storage/') || url.pathname.startsWith('/assets/') || url.pathname.startsWith('/decoders/')) {
        event.respondWith(
            caches.match(request).then((cached) => {
                const fetchPromise = fetch(request).then((response) => {
                    // status === 200 only — response.ok also accepts 206
                    // (Range responses), which cache.put rejects with
                    // "Partial response (status code 206) is unsupported".
                    // (Post-deploy hotfix v3.)
                    if (response.status === 200) {
                        const clone = response.clone();
                        caches.open(CACHE_NAME).then((cache) => cache.put(request, clone));
                    }
