...
class VenueFrontierTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_catalog_report_register_coverage_lists_the_gap(): void
    {
        ...
        // …intimacy is covered by The Salon…
        $intimacy = $coverage->firstWhere('register', 'intimacy');
        $this->assertNotNull($intimacy);
        $this->assertSame('covered', $intimacy['status'], 'The Salon covers the intimacy register.');
    ...
    public function test_try_on_flag_defaults_off(): void
    {
        $this->assertFalse(
            ...
            'Try-on ships opt-in: FEATURE_FLAG_VENUE_TRY_ON defaults to false.'
        );
    }
    ...
    public function test_preview_payload_carries_try_on_only_when_flag_on(): void
    {
        ...
        // Flag on: try-on arms on the preview payload.
    ...
    public function test_tryon_js_has_zero_network_surface(): void
    {
        ...
        foreach ([
            ...
            $this->assertStringNotContainsString(
                $forbidden,
                $source,
                "TryOn.js must not contain '{$forbidden}' — client-side only"
            );
        }
    }
