...
- **`.eyebrow` is the only overline label.** The `text-sm … uppercase tracking-wider` and weightless `text-xs uppercase` variants are retired. In the ops slate zone, keep utilities but match the canonical weight/size (`text-xs font-semibold uppercase tracking-wider`).
- **Weights:** ops/CC chips and uppercase section headings are `font-semibold`, never `font-bold`. Stat *values* may stay bold only where they were already sanctioned `text-2xl`; larger stat digits (`text-3xl`+) are `font-semibold text-numeric`. Numeric-heavy tables and counters carry `.text-numeric`.
- **`page-title` in the slate zone:** ops/CC h1s use `class="page-title text-slate-50"` — the utility beats the component's `text-gray-50`, so the ramp (20→24px, tracking, leading) is shared. No page may hand-roll `text-xl font-semibold` or `text-2xl font-bold` h1s again.
- **Toast durations are per-type:** success/info 3.5s, **error/warning 7s** (errors were the shortest-lived messages in the product). Toast copy is sentence-case with no trailing period; the clipboard action is always "Link copied". Never concatenate `err.message` into a user-facing toast.
- **Confirm dialogs support verbs:** a form may add `data-confirm-button="Delete"` (and `data-confirm-danger="false"` for neutral actions, which swaps the red confirm for a primary "Please Confirm" dialog). Default remains red "Confirm".
- **Empty states must pass the three-question test** (what is empty / why / what can I do). The cold one-liners ("No feedback yet.", "No galleries found.") are gone; super-admin cold states and the four public dead-ends now explain and link. `<x-empty-state>` is the sanctioned component — adopting it further requires no new design.
- **Dialog scroll containment:** every `fixed inset-0` dialog overlay carries `overflow-y-auto` so tall content is reachable on short viewports (pricing modals, feedback sheet, ban modal, confirm modal, upgrade modal, all hand-rolled page modals).
- **Accessibility wiring:** form labels and controls are associated (`for`/`id` or `aria-label` on filter selects), validation errors carry `id="{field}-error"` + `aria-invalid`/`aria-describedby` via the standard snippet, selection controls expose state (`aria-pressed`, `aria-expanded`), and every layout has a skip link + `<main>` landmark. Busy feedback (`data-busy data-busy-label`) is mandatory on slow submits — multipart upload forms included.
- **Dashboard stat contract:** one tile answers one question. Gallery counts appear once (Total, with live·draft as sub-line); the chart owns the trend; the quota detail lives in the right rail. Master Control is health-first: backup health above feature flags; configuration metadata collapses into `<details>`.
- **Ops incident hierarchy:** Impact → Likely cause → Timeline → Recommended next steps. Timeline severity chips map per-event, never from the incident's severity.
- **Shadow/radius discipline:** `rounded-2xl` and raw `shadow-lg/xl/2xl` in product UI are retired (welcome marketing hero excepted). Surfaces: `.card`+`shadow-card`; dialogs `.modal-panel`; floating layers `shadow-menu`; clickable cards `.card-interactive card-lift`.
