#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
// nebula-drift-qa.mjs — the venue QA gate for Nebula Drift ("The Deep Field").
//
...
//      rig, standing glow, restrained bloom, black vignette, depth bands,
//      light pools) and NOT the superseded v1.0.0 keys.
...
//      + three.js (no GL, canvas stubbed): the band shells precess in
//      opposite senses, stars are NEUTRAL (the v1.0.0 all-violet sky is
//      gone), the current carries its shader drift attributes, the ring
//      floats above eye level, monolith silhouettes stand beyond the field,
//      light pools land under the artworks, the body scales across the
//      capacity range, low-end keeps the composition, and two builds from
//      the same seed are transform-identical.
...
ok('ambient base wash 0.55 (at 40 works only 12 carry a pool light — the rest must read)',
    /'ambient_intensity'\s*=>\s*0\.55/.test(vc));
...
ok('rig luminous: spot 1.2 (pool target ≈ 4.2, void family)',
    /'spot_intensity'\s*=>\s*1\.2/.test(vc));
ok('standing glow declared (void family sits beyond the proximity radius)',
    /'artwork_light_base'\s*=>\s*0\.5/.test(vc));
...
if (hBody) {
    const pairs = [
        ...
        ['ambient_intensity 0.55',               /ambient_intensity:\s*0\.55/.test(hBody) && /'ambient_intensity'\s*=>\s*0\.55/.test(vc)],
        ['hemisphere_intensity 0.35',            /hemisphere_intensity:\s*0\.35/.test(hBody) && /'hemisphere_intensity'\s*=>\s*0\.35/.test(vc)],
        ['spot_intensity 1.2',                   /spot_intensity:\s*1\.2/.test(hBody) && /'spot_intensity'\s*=>\s*1\.2/.test(vc)],
        ...
        ['artwork_light_base 0.5',               /artwork_light_base:\s*0\.5/.test(hBody) && /'artwork_light_base'\s*=>\s*0\.5/.test(vc)],
...
try {
    ...
    ok('band built: 30 sprites on desktop (10 feature masses + 20 backbone haze)', sprites.length === 30, `got ${sprites.length}`);
    ok('band masses differ by seeded screen rotation (5 shared canvases, no clones)',
        new Set(sprites.map((s) => s.material.rotation)).size >= 25);
    ...
    ok('meridian ring present', meshes.length === 1);
    if (meshes.length === 1) {
        ok('ring floats above the walk band (overhead, not a tripwire)',
            meshes[0].position.y > 8, `y ${meshes[0].position.y.toFixed(1)}`);
        ok('ring tilted off-axis (found, not installed)',
            Math.abs(meshes[0].rotation.x) > 0.1);
    }

    // Sprite shells: the sky must be LAYERED (two distinct radii) and
    // precess in OPPOSITE senses (parallax), both subliminal (< 0.01 rad/s).
    const drifts = (probes[1].ctx._particleSystems || []).filter((p) => p.type === 'drift-rotate');
    ok('two band shells registered for precession', drifts.length === 2, `got ${drifts.length}`);
    ok('shells precess in OPPOSITE senses (layered parallax)',
        drifts.length === 2 && Math.sign(drifts[0].speed) !== Math.sign(drifts[1].speed));
    ...
    ok('band is LAYERED (far shell ~2.5-4× the venue radius, mid shell nearer)',
        minD > probes[1].r * 1.6 && maxD < probes[1].r * 4.2,
    ...
    if (instanced.length === 1) {
        ...
        ok('monoliths are dark silhouettes (luminance ≤ 0.06)',
            inst.material.color.getHSL({}).l <= 0.06 || (() => { const h = {}; inst.material.color.getHSL(h); return h.l <= 0.06; })());
    ...
    // Low-end: composition survives, counts shrink, near anchors drop.
    {
        ...
        ok('low-end: the band still builds (identity survives degradation)',
            lo.sprites.length === 10, `got ${lo.sprites.length} (features only — the haze backbone is a fill-rate luxury)`);
    ...
    // declared (same pure planner the pool anchors read).
    {
...
ok('no legacy purple ambient in the row (0x8844ff purged from the rig)',
    !/0x8844ff/.test(row));
ok('no all-violet star palette in the body (hue 0.7–0.85 as a RANGE is gone)',
    !addNebulaDeepfieldSrc().match(/0\.7\s*\+/));
...
// Migration: guarded pattern present.
const migPath = 'database/migrations/2026_09_08_000002_nebula_drift_deepfield.php';
ok('guarded migration ships', existsSync(rel(migPath)));
if (existsSync(rel(migPath))) {
    const mig = readFileSync(rel(migPath), 'utf8');
    ok('migration swaps the exact seeded fixture list (admin edits win)',
        mig.includes('OLD_FIXTURES') && mig.includes('NEW_FIXTURES'));
    ok('migration guards the copy swap on an exact match',
        mig.includes('OLD_DESCRIPTION') && mig.includes('NEW_DESCRIPTION'));
    ok('migration version swap exact-match guarded (1.0.0 → 2.0.0)',
        mig.includes("'1.0.0'") && mig.includes("'2.0.0'"));
    ok('migration has a matching down() restore',
